...
const fieldClass = "h-[46px] w-full rounded-[5px] border border-[#dfdfdf] bg-white px-4 text-[14px] text-ink focus:outline-2 focus:outline-navy";
...
function ShippingEstimator({ subtotal }: { subtotal: number }) {
  ...
  return (
    <div className="w-full max-w-[500px] rounded-[5px] bg-[#fff5fb] p-[25px]">
  ...
  return (
    <div className="container-page pt-[49px]">
      <div className="flex items-center justify-between">
        ...
        <Link href="/" className="text-[15px] font-bold text-magenta hover:underline">Seguir comprando</Link>
      </div>

      {!ready ? (
        <div className="mt-5 h-[400px] animate-pulse rounded-[5px] bg-[#fff5fb]" />
      ) : lines.length === 0 ? (
        <div className="mt-5 rounded-[5px] px-6 py-20 text-center shadow-[0_0_0_1px_#dfdfdf]">
          ...
          <Link href="/" className="mt-6 inline-grid h-11 place-items-center rounded-[5px] bg-navy px-8 text-[14px] font-bold text-white hover:bg-navy-dark">
            Descubrir productos
          </Link>
        </div>
      ) : (
        <div className="mt-5 grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
          ...
          <aside className="rounded-[5px] bg-[#fff5fb] p-[25px]">
            <label htmlFor="notes" className="block text-[15px] leading-[18px] text-muted">Instrucciones especiales del pedido</label>
            <textarea id="notes" value={note} onChange={(e) => setNote(e.target.value)} placeholder="Ej. mensaje para la tarjeta de dedicatoria" className="mt-2 block h-[140px] w-full resize-y rounded-[5px] border border-[#dfdfdf] bg-white p-3 text-[14px] text-ink focus:outline-2 focus:outline-navy" />
            ...
            <form onSubmit={applyCoupon} className="mt-[25px] flex h-[46px] rounded-[5px] border border-[#dfdfdf] bg-white p-[3px]">
              ...
              <button disabled={checking} className="h-full rounded-[3px] bg-navy px-5 text-[15px] font-bold text-white hover:bg-navy-dark disabled:opacity-60">{checking ? "…" : "Aplicar"}</button>
            </form>
            ...
            {coupon && !couponMessage && (
              <p className="mt-2 flex items-center justify-between text-[12px] text-muted">
                ...
                <button onClick={() => setCoupon("")} className="text-magenta underline underline-offset-4">Quitar</button>
              </p>
            )}
            ...
            <p className="mt-[5px] text-[12px] leading-[18px] font-bold text-muted">
              ...
              {freeShipping.missing > 0 && (
                <>– <Link href="/" className="text-magenta hover:underline">Seguir comprando</Link></>
              )}
            </p>
            ...
            <CheckoutButton accepted={accepted} className="mt-[25px] h-[46px] w-full rounded-[5px] bg-navy text-[14px] font-bold text-white hover:bg-navy-dark" />
          </aside>
        </div>
      )}
    </div>
