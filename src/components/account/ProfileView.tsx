    ...
    <>
      ...
      <section className="mt-6 rounded-[5px] bg-white p-6">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="grid size-8 place-items-center rounded-[5px] text-magenta hover:bg-muted/5"
          >
      ...
      <section className="mt-5 rounded-[5px] bg-white p-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => openAddress()} className="flex items-center gap-1 text-[14px] font-bold text-magenta hover:underline">
              ...
              <li key={a.id} className="rounded-[5px] p-4 text-[14px] leading-[1.5] text-muted shadow-[inset_0_0_0_1px_#dfdfdf]">
                <div className="flex items-start justify-between gap-2">
                  <p className="flex items-center gap-1.5 font-bold text-ink">
                    <MapPin className="size-4 text-magenta" strokeWidth={1.5} />
                    {a.firstName} {a.lastName}
                  </p>
                  {a.isDefault && <span className="rounded-full bg-[#fdf3f8] px-2 py-0.5 text-[12px] font-bold text-magenta">Predeterminada</span>}
                </div>
                <p className="mt-1">
                  {a.address1}
                ...
                <div className="mt-3 flex gap-4 text-[13px]">
                  <button onClick={() => openAddress(a)} className="flex items-center gap-1 text-ink hover:text-magenta"><Pencil className="size-3.5" /> Editar</button>
        ...
        <Modal title="Editar perfil" onClose={() => setEditingProfile(false)}>
          ...
          >
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button disabled={saving} className="h-11 rounded-[5px] bg-navy px-6 text-[14px] font-bold text-white hover:bg-navy-dark disabled:opacity-60">{saving ? "Guardando…" : "Guardar"}</button>
            </div>
          </form>
        </Modal>
      )}
        ...
        <Modal title={customer.addresses.some((a) => a.id === editingAddress.id) ? "Editar dirección" : "Agregar dirección"} onClose={() => setEditingAddress(null)}>
          ...
          >
            ...
            <label className="mt-4 flex items-center gap-2 text-[14px] text-ink">
              <input type="checkbox" checked={editingAddress.isDefault} onChange={(e) => setEditingAddress({ ...editingAddress, isDefault: e.target.checked })} className="size-4 accent-navy" />
